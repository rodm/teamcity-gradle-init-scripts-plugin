...
class SameInitScriptsHealthReportTest {
    ...
    void setup() {
        ...
        when(descriptor.getPluginResourcesPath(eq('/health/sameInitScripts.jsp'))).thenReturn('pluginResourcesPath/health/sameInitScripts.jsp')
        report = new SameInitScriptsHealthReport(places, descriptor)
    }

    @Test
    void 'health status report configuration'() {
        assertThat(report.getType(), equalTo('SameInitScriptsReport'))
        assertThat(report.getDisplayName(), equalTo('Same Gradle init script selected in build step and build feature'))
        assertThat(report.getCategories(), hasItem(new ItemCategory('same_init_scripts', 'Same Gradle init script selected in build step and build feature', WARN)))
    }
    ...
    void 'health status report registers extension page'() {
        ...
        assertThat(extension.getIncludeUrl(), equalTo('pluginResourcesPath/health/sameInitScripts.jsp'))
    ...
    @Test
    void 'build type with build runner and build feature using the same init script is reported'() {
        ...
        assertThat(item.getIdentity(), equalTo('same_init_scripts_BuildTypeId'))
    ...
    @Test
    void 'build type with build runner and build feature using different init scripts is not reported'() {
        SBuildRunnerDescriptor runner = mock(SBuildRunnerDescriptor)
        when(runner.getParameters()).thenReturn([(INIT_SCRIPT_NAME_PARAMETER): 'runner.gradle'])
        SBuildFeatureDescriptor feature = mock(SBuildFeatureDescriptor)
        when(feature.getParameters()).thenReturn([(INIT_SCRIPT_NAME): 'feature.gradle'])
        SBuildType buildType = mock(SBuildType)
        when(buildType.getBuildTypeId()).thenReturn('BuildTypeId')
        when(buildType.getBuildRunners()).thenReturn([runner])
        when(buildType.getBuildFeaturesOfType(FEATURE_TYPE)).thenReturn([feature])
    ...
    @Test
    void 'build template with build runner and build feature using the same init script is reported'() {
        SBuildRunnerDescriptor runner = mock(SBuildRunnerDescriptor)
        when(runner.getParameters()).thenReturn([(INIT_SCRIPT_NAME_PARAMETER): 'init.gradle'])
        SBuildFeatureDescriptor feature = mock(SBuildFeatureDescriptor)
        when(feature.getParameters()).thenReturn([(INIT_SCRIPT_NAME): 'init.gradle'])
        ...
        HealthStatusItem item = itemCaptor.value
        assertThat(item.getIdentity(), equalTo('same_init_scripts_BuildTemplateId'))
        assertThat(item.getSeverity(), equalTo(WARN))
    ...
    @Test
    void 'build template with build runner and build feature using different init scripts is not reported'() {
        SBuildRunnerDescriptor runner = mock(SBuildRunnerDescriptor)
        when(runner.getParameters()).thenReturn([(INIT_SCRIPT_NAME_PARAMETER): 'runner.gradle'])
        SBuildFeatureDescriptor feature = mock(SBuildFeatureDescriptor)
        when(feature.getParameters()).thenReturn([(INIT_SCRIPT_NAME): 'feature.gradle'])
